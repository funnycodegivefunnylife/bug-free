...
public class BookingController {
  ...
  @Operation(
      ...
      description = "Books a room for the specified time period")
  ...
  public ResponseEntity<String> createBooking(
    ...
    if (success) {
      ...
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body("Unable to create booking - room may be unavailable");
    }
  }
  ...
  public ResponseEntity<ListBookingsResponse> listBookings(
    ...
    ListBookingsUseCase.ListBookingsRequest request =
        new ListBookingsUseCase.ListBookingsRequest(page, size, userId, roomId);
    ListBookingsUseCase.ListBookingsResponse response = listBookingsUseCase.execute(request);

    List<BookingDto> bookingDtos = response.bookings().stream().map(bookingMapper::toDto).toList();

    ListBookingsResponse apiResponse =
        new ListBookingsResponse(
            bookingDtos,
            response.totalCount(),
            response.page(),
            response.size(),
  ...
  public ResponseEntity<BookingDto> updateBooking(
    ...
    UpdateBookingUseCase.UpdateBookingRequest request =
        bookingMapper.toUpdateRequest(bookingRequestDto);
  ...
  public record ListBookingsResponse(
      List<BookingDto> bookings, long totalCount, int page, int size, int totalPages) {}
}
