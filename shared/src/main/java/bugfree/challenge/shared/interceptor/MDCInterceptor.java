...
/**
 * Interceptor to handle MDC context for request processing This interceptor ensures request ID is
...
public class MDCInterceptor implements HandlerInterceptor {
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
    ...
    logger.debug(
        "Processing request: {} {} with request ID: {}",
        request.getMethod(),
  ...
  public void postHandle(
    ...
    String requestId = RequestIdMDC.getRequestId();
    logger.debug(
        "Completed processing request: {} {} with request ID: {}",
        request.getMethod(),
        request.getRequestURI(),
        requestId);
  }
  ...
  public void afterCompletion(
    ...
    if (ex != null) {
      String requestId = RequestIdMDC.getRequestId();
      logger.error(
          "Exception occurred during request processing for request ID: {}", requestId, ex);
    }

    // Note: MDC cleanup is handled by RequestIdFilter to ensure it happens after all
